...
export async function GET(req: Request) {
  try {
    ...
    const callbackUrl = `${AUTH_SERVER_URL}/api/developer/oauth/callback?code=${code}&state=${state || ""}&provider=google`;
    console.log("[Google OAuth] Calling auth server:", callbackUrl);

    const res = await fetch(callbackUrl, {
      redirect: "manual",
    });

    console.log("[Google OAuth] Auth server response status:", res.status);
    console.log(
      "[Google OAuth] Auth server response headers:",
    ...
    console.log("[Google OAuth] Set-Cookie header:", setCookie);
    console.log("[Google OAuth] Location header:", location);

    if (!setCookie || !location) {
      console.error("[Google OAuth] Missing cookie or location");
      return NextResponse.redirect(
    ...
    console.log("[Google OAuth] Cookie name:", name);
    console.log("[Google OAuth] Cookie value:", value);
    ...
    console.log(
      "[Google OAuth] Returning HTML with meta refresh and Set-Cookie header",
    );
    ...
    console.error("[Dashboard Google OAuth Callback Error]", error);
    const url = new URL(req.url);
    return NextResponse.redirect(
